'''
After Joel bought a boat he have got the new idea - to buy a house. Since he did not want to wait until necessary sum
is collected, he considers a mortgage, which is to get a loan from bank and buy a house immediately, while loan is
secured by this house (i.e., in case of borrower's default bank gets the house in compensation).
...
For example, Joel takes P = $800,000 from bank with interest rate of R = 6%. He is willing to pay M = $10000 at the end
of each month.
...
def monthly_payment(p, r, l):
    r = r * 0.01
    return ceil(p * (r / 12) / (1 - (1 + r / 12) ** (-l)))


print(monthly_payment(*map(int, input().split())))
